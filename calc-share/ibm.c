/****************************************************/
/* File: ibm.c                                      */
/* Modified from the TM by Kenneth C. Louden        */
/****************************************************/
  ...
  while (! feof(pgm)) {
    ...
    if ( (nonBlank()) && (in_Line[inCol] != '*') ) {
      ...
      case opclRA :
        ...
        arg3 = num;
	if ( ! skipCh(')'))
            return error("Missing RParen", lineNo,loc);
        break;
      }
      iMem[loc].iop = op;
      iMem[loc].iarg1 = arg1;
...
int doCommand (void) {
  ...
  case 'h' :
    ...
    printf("Commands are:\n");
    printf("   s(tep <n>      "\
	   "Execute n (default 1) IBM instructions\n");
    printf("   g(o            "\
	   "Execute IBM instructions until HALT\n");
    printf("   r(egs          "\
...
int main( int argc, char * argv[] ) {
  ...
  /* reset( input ); */
  /* read-eval-print */
  printf("IBM  simulation (enter h for help)...\n");
  do
     done = ! doCommand ();
  while (! done );
